...
static request deserialize_req(byte_array msg) {
    ...
    if (!from_s || !to_s || !amount_s) return ZERO_REQ;
    if (strncmp(from_s, "from=", strlen("from=")) ||
        strncmp(to_s, "to=", strlen("to=")) ||
        strncmp(amount_s, "amount=", strlen("amount="))) return ZERO_REQ;
    request req;
    errno = 0;
    req.from = strtol(from_s+strlen("from="), NULL, 10);
    if (errno) return ZERO_REQ;
